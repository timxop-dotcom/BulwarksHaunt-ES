...
namespace BulwarksHaunt
{
    public static class TMProEffects
    {
        ...
        {
            orig(self);
            var component = self.messagesText.textComponent.GetComponent<BulwarksHauntTextEffects>();
            if (!component)
            {
                component = self.messagesText.textComponent.gameObject.AddComponent<BulwarksHauntTextEffects>();
            }
        }
        ...
        {
            orig(self);
            var component = self.GetComponent<BulwarksHauntTextEffects>();
            if (!component)
            {
                component = self.gameObject.AddComponent<BulwarksHauntTextEffects>();
                component.textComponent = self;
            }
        }

        public class BulwarksHauntTextEffects : MonoBehaviour
        {
            ...
            public void Update()
            {
                ...
                while (updateTimer <= 0f)
                {
                    ...
                    if (textComponent && textComponent.isActiveAndEnabled)
                    {
                        ...
                        foreach (var link in textInfo.linkInfo.Where(x => x.GetLinkID() == "BulwarksHauntWavy"))
                        {
                        ...
                        foreach (var link in textInfo.linkInfo.Where(x => x.GetLinkID() == "BulwarksHauntRainbowWavy"))
                        {
                            for (int i = link.linkTextfirstCharacterIndex; i < link.linkTextfirstCharacterIndex + link.linkTextLength; i++)
                            {
                                var charInfo = textInfo.characterInfo[i];
                                ...
                                {
                                    destVerts[charInfo.vertexIndex + j] = origVerts[charInfo.vertexIndex + j] + charOffset;
                                    destColors[charInfo.vertexIndex + j] = Color.HSVToRGB((Time.time * 0.15f + 0.06f * i) % 1f, 0.85f, 1f);
                                }
                            }
                        }

                        foreach (var link in textInfo.linkInfo.Where(x => x.GetLinkID() == "BulwarksHauntShaky"))
                        {
                            for (int i = link.linkTextfirstCharacterIndex; i < link.linkTextfirstCharacterIndex + link.linkTextLength; i++)
                            {
                                var charInfo = textInfo.characterInfo[i];
